...
client.on("guildMemberAdd", async member => {
  const moment = require('moment')
   ...
   if (!kanal) return;
   const targetMember = member.guild.members.cache.get(member.id);
   if (!targetMember) return;
   if (kayıtlı) {
    await targetMember.roles.add(kayıtlı).catch(() => {});
  }
   const targetChannel = client.channels.cache.get(kanal);
   if (!targetChannel) return;
   targetChannel.send(`${member} Seni aramızda görmekten mutluluk duyuyoruz!\n\nSunucumuza katılan **${member.guild.memberCount}.** üyesin.\n\nHesabın **${kuruluş}** tarihinde kurulmuştur.\n\nSunucumuzun kurallarını okumayı unutma iyi eğlenceler.`).catch(() => {});
})
